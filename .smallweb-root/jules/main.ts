...
const defaultSessionsFile = `${Deno.cwd()}/data/sessions.jsonl`
...
async function appendSession(record: SessionRecord): Promise<void> {
  ...
  const sessionsPath = typeof sessionsFile === "string" ? sessionsFile : sessionsFile.pathname
  const lastSlash = sessionsPath.lastIndexOf("/")

  if (lastSlash > 0) {
    const sessionsDir = sessionsPath.slice(0, lastSlash)
