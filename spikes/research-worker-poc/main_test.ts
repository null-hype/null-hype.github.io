...
Deno.test("worker session start calls back to mcp and stores the completed session", async () => {
  ...
  const callbackServer = Deno.serve({
    ...
    callbackPayloads.push(JSON.parse((await req.text()).trim()))
...
Deno.test("worker session start can invoke target.start_service before recording a note", async () => {
  const calls: string[] = []
  const mcpPort = getAvailablePort()
  const callbackPort = getAvailablePort()
  const abortController = new AbortController()
  ...
  const mcpServer = Deno.serve({
    ...
    if (payload.params?.name === "target.start_service") {
      return new Response(JSON.stringify({
        id: "3",
        jsonrpc: "2.0",
        result: {
          structuredContent: {
            endpoint: "lunary-app:3333",
            mode: "baseline",
          },
        },
      }), {
  ...
  try {
    ...
    await waitFor(async () => {
      ...
      const session = await sessionResponse.json()
      assert.equal(session.status, "completed")
      assert.match(session.summary, /via lunary-app:3333\./)
    })

    assert.equal(callbackPayloads.length, 1)
    assert.equal(callbackPayloads[0].status, "completed")
    assert.match(callbackPayloads[0].summary, /via lunary-app:3333\./)
  } finally {
    abortController.abort()
    callbackAbort.abort()
    await Promise.allSettled([mcpServer.finished, callbackServer.finished])
