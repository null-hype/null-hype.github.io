...
export class TidelaneInfra {
  ...
  async smallwebBundle(
    ...
    return dag.directory()
      .withDirectory(".smallweb-root", await this.renderSmallwebRoot(smallwebRoot, adminAuthorizedEmails, additionalDomains))
  ...
  async smallwebMutagenProject(
    ...
    return dag.directory()
      .withDirectory("bundle", await this.smallwebBundle(smallwebRoot, dist, adminAuthorizedEmails, additionalDomains))
  ...
  private async renderSmallwebRoot(smallwebRoot: Directory, adminAuthorizedEmails: string, additionalDomains: string): Promise<Directory> {
    ...
    const extraDomains = this.parseAdditionalDomains(additionalDomains)
    ...
    if (extraDomains.length > 0) {
      config.additionalDomains = Array.from(new Set([...(config.additionalDomains ?? []), ...extraDomains]))

      const appAliases: Record<string, string[]> = {}
      for (const domain of extraDomains) {
        appAliases.www = [...(appAliases.www ?? []), domain, `www.${domain}`]
        appAliases.admin = [...(appAliases.admin ?? []), `admin.${domain}`]
        appAliases["null-hype"] = [...(appAliases["null-hype"] ?? []), `null-hype.${domain}`]
        appAliases.jules = [...(appAliases.jules ?? []), `jules.${domain}`]
        appAliases["linear-agent"] = [...(appAliases["linear-agent"] ?? []), `linear-agent.${domain}`]
      }
      ...
      for (const [appName, aliases] of Object.entries(appAliases)) {
        ...
        config.apps[appName] = {
          ...appConfig,
          additionalDomains: Array.from(new Set([...(appConfig.additionalDomains ?? []), ...aliases])),
        }
      }
    }
  ...
  private parseAdditionalDomains(value: string): string[] {
    return value
      .split(/[\n,]/)
      .map((entry) => entry.trim())
      .filter((entry) => entry !== "")
